...
:: 如果从音乐目录下找到的 MP3 文件数量小于等于 3，则将播放器设置为使用豆瓣电台播放，否则使用 VLC 
if %Count% LEQ 3 (
    echo 使用豆瓣电台
    ...
    echo 使用 VLC
    taskkill /f /im vlc.exe >nul 2>nul
    ...
    start /min "" "C:\Program Files\VideoLAN\VLC\vlc.exe" --qt-start-minimized --playlist-tree --no-qt-privacy-ask --random "/musicbak/Playlists/自动播放列表.xspf"
    ping -n 5 127.0.0.1 >nul 2>nul
    echo 开始自动播放
    ping -n 900 127.0.0.1 >nul 2>nul

    :: 停止 VLC 的运行，并删除已经播放过的文件
    taskkill /F /IM vlc.exe >nul 2>nul
